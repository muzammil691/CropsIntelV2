...
export default function Reports() {
  ...
  const exportCSV = () => {
    const headers = ['Period','Crop Year','Supply (lbs)','Dom Shipped','Exp Shipped','Total Shipped','Dom Committed','Exp Committed','Total Committed','New Dom Commit','New Exp Commit','Total New Commit','Uncommitted','Carry In','Receipts'];
    const rows = filtered.map(r => [
      `${r.report_year}/${String(r.report_month).padStart(2,'0')}`,
      r.crop_year,
      r.total_supply_lbs, r.domestic_shipped_lbs, r.export_shipped_lbs, r.total_shipped_lbs,
      ...
      r.uncommitted_lbs, r.carry_in_lbs, r.receipts_lbs
    ]);
    ...
    a.href = url;
    a.download = `cropsintel_position_reports_${selectedCrop === 'all' ? 'all' : selectedCrop.replace('/', '-')}.csv`;
    a.click();
